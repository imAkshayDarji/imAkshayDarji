<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=00D9FF&center=true&vCenter=true&width=700&lines=Hi!+I%27m+Akshay+Darji;AI+Engineer;LLM+Orchestration+%7C+RAG+Architectures;Autonomous+Agents+%7C+Production+AI+Systems)](https://git.io/typing-svg)

### Learning AI through hands-on projects.

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://portfolio-akshay-darji.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/imakshaydarji/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:akshaysdarji@gmail.com)

![Location](https://img.shields.io/badge/📍_Sheffield,_UK-222222?style=flat-square)
![Open to Work](https://img.shields.io/badge/Open_To_Work-AI_Engineer_|_LLM_Engineer_|_ML_Engineer-00c853?style=flat-square)

</div>

---

# 🤖 About Me

```python
akshay = {
    "role"       : "AI Engineer / AI Automation Engineer",
    "location"   : "Sheffield, UK 🇬🇧",
    "education"  : "MSc Artificial Intelligence — Sheffield Hallam University (Merit)",
    "focus"      : [
        "LLM Orchestration",
        "RAG Architectures",
        "Autonomous Agents"
    ],
    "stack"      : [
        "Python",
        "FastAPI",
        "PostgreSQL",
        "pgvector",
        "Docker",
        "AWS"
    ],
    "mantra"     : "Production AI systems — not demos."
}
```

I build **autonomous AI systems** that ship to production and stay operable.

My work spans:

* RAG pipelines with vector retrieval
* Multi-LLM orchestration
* Browser automation fleets
* Hardened FastAPI backends
* Production telemetry & observability

So stakeholders always know:

* what ran
* what retrieved
* what failed
* and what to replay when systems drift

* 🔭 Currently building autonomous agents & production RAG systems
* 🎓 MSc Artificial Intelligence — Sheffield Hallam University
* 🚀 Passionate about grounded LLMs & resilient AI infrastructure
* 📫 Reach me: **[akshaysdarji@gmail.com](mailto:akshaysdarji@gmail.com)**

---

# 🚀 Flagship Projects

<table>
<tr>

<td width="50%" valign="top">

## 🤖 AutoApply AI

### Autonomous AI Recruitment Platform

Processes **100+ job applications/hour** via multi-LLM orchestration.

### Key Highlights

* 95%+ ATS keyword match rate
* Reduced application time by 97%
* Autonomous browser automation
* RAG-powered CV optimization
* Multi-agent execution pipelines

### Stack

`FastAPI` `Redis` `Playwright` `PostgreSQL` `RAG` `pgvector`

### Repository

🔗 [JobedIn-v3](https://github.com/imAkshayDarji/JobedIn-v3)

</td>

<td width="50%" valign="top">

## 💬 Krystal Studio

### Production Multilingual AI Chat Platform

24/7 RAG-grounded support platform supporting:

* English
* Hindi
* Gujarati

### Features

* Hybrid dense + lexical retrieval
* HALFVEC / HNSW vector search
* JWT admin dashboard
* Safety routing
* Lead capture pipelines
* Production CI/CD

### Stack

`Next.js` `FastAPI` `PostgreSQL` `pgvector`

### Repository

🔗 [AI_ChatBot](https://github.com/imAkshayDarji/AI_ChatBot)

</td>

</tr>

<tr>

<td width="50%" valign="top">

## 🔍 Research Expert Finder

### Sheffield Hallam University

Research discovery platform for the SHU Department of Computing.

### Features

* Semantic expert search
* Vector similarity retrieval
* Fast filtering UX
* Research profile indexing

### Stack

`Next.js` `FastAPI` `pgvector` `Semantic Search`

### Repository

🔗 [GODL1KE](https://github.com/imAkshayDarji/GODL1KE)

</td>

<td width="50%" valign="top">

## 🧠 Portfolio Projects

Explore additional shipped systems including:

* Computer Vision
* NLP
* Deep Learning
* AI Automation
* ML Experiments

### Portfolio

🌐 [portfolio-akshay-darji.vercel.app/projects](https://portfolio-akshay-darji.vercel.app/projects)

</td>

</tr>
</table>

---

# 🛠️ Tech Stack

## 🤖 AI / ML

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white)
![LlamaIndex](https://img.shields.io/badge/LlamaIndex-6E57E0?style=for-the-badge)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-00C853?style=for-the-badge)
![LLMs](https://img.shields.io/badge/LLMs-8E24AA?style=for-the-badge)
![AI Agents](https://img.shields.io/badge/AI_Agents-FF6B35?style=for-the-badge)

---

## ⚙️ Backend & APIs

![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-005571?style=for-the-badge)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)

---

## 🗄️ Databases & Vector Search

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![pgvector](https://img.shields.io/badge/pgvector-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)

---

## 🌐 Frontend

![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=for-the-badge&logo=tailwind-css&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

---

## 🤖 Automation & DevOps

![Playwright](https://img.shields.io/badge/Playwright-2EAD33?style=for-the-badge&logo=playwright&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI/CD-FF9800?style=for-the-badge)

---

# 🧭 What I'm Working On

```text
🔬 Extending AutoApply AI → more ATS connectors + explainable run histories
🧠 Exploring multi-agent architectures with tool-use & memory
🌐 Open to AI Engineer · LLM Engineer · ML Engineer opportunities
```

---

# 💼 Open To Opportunities

I'm actively looking for roles in:

* 🤖 AI Engineer / AI Automation Engineer
* 🧠 LLM Engineer / ML Engineer
* 🌍 On-site (UK), Hybrid, or Remote

### 📬 Contact

* Email: [akshaysdarji@gmail.com](mailto:akshaysdarji@gmail.com)
* Portfolio: [portfolio-akshay-darji.vercel.app](https://portfolio-akshay-darji.vercel.app/)
* LinkedIn: [linkedin.com/in/imakshaydarji](https://www.linkedin.com/in/imakshaydarji/)
